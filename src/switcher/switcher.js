...
const Switcher = ({buttonHandler}) => {
    ...
    return(
        <div>
            <button type="button" className="btn btn-danger" onClick={() =>(buttonHandler(smalUrl))}>Danfer</button>
            <button type="button" className="btn btn-warning" onClick={() =>(buttonHandler(bigUrl))}>Warning</button>
        </div>
    )
}
